...
function FindProxyForURL(url, host) {
    ...
    // List of domains that shouldnot be routed through the proxy.
    ...
    // Do not proxy the blacklisted domains, and use direct connection
    if (isHostMatch(host, blacklistedDomains)) {
        return directConnection;
    }

    // Default to proxy connection if no match is found
    return proxyServer;
}
